    ...
    for i = index:stride:length(intersections)
        ...
        if intersections[i] <= 0 || (ray_color[i,1] + ray_color[i,2] + ray_color[i,3]) < 0.1 
            ...
            elseif objects[i,1] == 2
                ...
                # Diffusively reflect off ground (now computes the surface normal instead of assuming [0,0,1], like the original,
                #                                 but the shadows look a bit weird and I'm pretty sure there's a minor bug)
                total2 = 2
                while total2 > 1
                    xr, yr, zr = 2*rand()-1, 2*rand()-1, 2*rand()-1
                    total2 = xr*xr + yr*yr + zr*zr
                ...
                nx = V_tri[1,2,j]*V_tri[2,3,j] - V_tri[1,3,j]*V_tri[2,2,j]
                ny = V_tri[1,3,j]*V_tri[2,1,j] - V_tri[1,1,j]*V_tri[2,3,j]
                nz = V_tri[1,1,j]*V_tri[2,2,j] - V_tri[1,2,j]*V_tri[2,1,j]
                total2 = sqrt(nx*nx + ny*ny + nz*nz)
                nx /= total2
                ny /= total2
                nz /= total2
...
# Note: Triangle vertices MUST follow the right-hand rule! (counter-clockwise vertices)
#       Otherwise surface normals will be in the wrong direction and break bounces
#       There are solutions to this, but they are needlessly complicated (I just want to use cross products...)
